...
module.exports = async (ctx, next) => {
  ...
  if (ctx.request && ctx.request.header && ctx.request.header.authorization) {
    ...
    if (!ctx.state.user) {
      ...
      ctx.state.user = await strapi.services.administrator.findOne({ _id: id1 });

      if (!ctx.state.user) {
        ctx.state.user = await strapi.services.redactor.findOne({ _id: id1 });

        if (!ctx.state.user) {
          ctx.state.user = await strapi.services.student.findOne({ _id: id1 });
