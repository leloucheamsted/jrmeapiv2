    ...
    if (ctx.request && ctx.request.header && ctx.request.header.authorization) {
        ...
        try {
            ...
            if (!ctx.state.user) {
                ...
                ctx.state.user = await strapi.services.administrator.findOne({ _id: id });

                if (!ctx.state.user) {
                    ctx.state.user = await strapi.services.redactor.findOne({ _id: id });

                    if (!ctx.state.user) {
                        ctx.state.user = await strapi.services.student.findOne({ _id: id });
